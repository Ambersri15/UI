...
public class Ast5thReviewMilestoneGMB extends PreRequisites {
	
	public static Logger LOGS = LogManager.getLogger(Ast5thReviewMilestoneGMB.class.getName());
	...
	public void Assistant_Ast5thReviewMilestoneGMB(String BusinessId, String RepName, String Resultsfolderpath) throws Exception
	{
		...
		//--------------- 5th Review Milestone GMB -----------------------------
		
		if(astAssetId != null)
		{
			TriggerRestCall_5thReviewMilestoneGMB(astAssetId, BusinessId, Resultsfolderpath);
		}
		else
		{
	...
	//--------------- Trigger 5th Review Milestone GMB -----------------------------
		
	public void TriggerRestCall_5thReviewMilestoneGMB(String assetID, String BusinessId, String Resultsfolderpath)
			throws Exception {
		...
		String jsonData5thReviewMilestoneGoogle = "{\"eventType\":\"Review\"," + "\"eventDetails\":{" + "\"partnerLocationId\":\""
		...
		request.body(jsonData5thReviewMilestoneGoogle);
		...
		if ((statusCode == 200) || statusCode == 201) {
			...
			ExpRes = "User Should be able to Trigger "+ i +" Review Notification for Google";
			ActRes = "User Successfully Triggered "+ i +" Review Notification for Google";
			Status = "Pass";
			ExportResults.exportTestResult(
			...
			Status = "Fail";
			ExportResults.exportTestResult(
					"Review Notification:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName,
					Resultsfolderpath);
	   		}
		}
		...
		if(counter==5)
		{
			System.out.println("REST Call Executed 5 Times!!!............");
			
			RestAssured.baseURI = "http://dashboarduatbe.hibu.int";
			RequestSpecification request2 = RestAssured.given()
					.config(RestAssured.config().sslConfig(new SSLConfig().relaxedHTTPSValidation()));
			
			String jsonDataProcessMilestones = "{}";
			request2.header("Content-Type", "application/json");
			request2.body(jsonDataProcessMilestones);
			
			Response response2 = null;
			response2 = request2.post("/hibuassistant/processMilestones");
			
			int statusCode2 = response2.getStatusCode();
			LOGS.info("Status: " + statusCode2);

			String responseBody2 = response2.getBody().asString();
			LOGS.info("Output: " + responseBody2);

			if ((statusCode2 == 200) || statusCode2 == 201) {
				System.out.println("Response Success with :" + statusCode2);
				//System.out.println("Response Body is: " + responseBody);
				counter = counter +1;

				ExpRes = "User Should be able to Trigger processMilestones";
				ActRes = "User Successfully Triggered processMilestones";
				Status = "Pass";
				ExportResults.exportTestResult(
						"Process Milestones:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName, Resultsfolderpath);
				
				VerifyNotificationDetailsInDBfor5thReviewMilestoneGMB(BusinessId, Resultsfolderpath);

				}
				else
		   		{
		   		ExpRes = "User Should be able to Trigger processMilestones";
				ActRes = "There is Some Issue while Triggerring processMilestones";
				Status = "Fail";
				ExportResults.exportTestResult(
						"Process Milestones:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName,
						Resultsfolderpath);
		   		}
	...
	public void VerifyNotificationDetailsInDBfor5thReviewMilestoneGMB(String BusinessId, String Resultsfolderpath) throws Exception
	{
		LOGS.info("Business Id: " + BusinessId);
		LOGS.info("Inside VerifyNotificationDetailsInDBfor5thReviewMilestoneGMB");
			...
			if (document.containsValue(BusinessId)) {
				...
				try {
					...
					if (document.get("dateTime").toString().contains(presentdate)) {
						...
						if (document.get("eventType").toString().equalsIgnoreCase("ReviewMilestone")
								&& document.get("email").toString().contains("Your 5th Google review just came in")) {
							...
							ExpRes = "MongoDB: hibuAssistantNotificationDetails - EventType Should be Set to ReviewMilestone";
							ActRes = "MongoDB: hibuAssistantNotificationDetails - EventType is set as ReviewMilestone: "
									+ document.get("eventType")+" And 5th Google Review Email Triggered";
