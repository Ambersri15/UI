...
public class AstSignPostFeedback extends PreRequisites {

	public static Logger LOGS = LogManager.getLogger(AstSignPostFeedback.class.getName());
	...
	public void Assistant_SignPostFeedback(String BusinessId, String RepName, String Resultsfolderpath) throws Exception
	{
		...
		if(astAssetId != null)
		{
			TriggerRestCall_Feedback(astAssetId, BusinessId, Resultsfolderpath);
		}
		else
		{
	...
	public void TriggerRestCall_Feedback(String assetID, String BusinessId, String Resultsfolderpath) throws Exception
	{
...
//	 String baseURL = "https://hibuapigw.testingnet.com";
//	 String finalURL = baseURL+"/assistant/set-event-details";
	 
		
	RestAssured.baseURI ="https://hibuapigw.testingnet.com";
	RequestSpecification request = RestAssured.given().config(RestAssured.config().sslConfig(new SSLConfig().relaxedHTTPSValidation()));
	...
	String randomValue = String.valueOf(randomNum);
   	 ...
   	 String jsonData = "{\"eventType\":\"Feedback\","
   			+ "\"eventDetails\":{"
   			+ "\"partnerLocationId\":\""+assetID+"\","
   			+ "\"timestamp\":\""+timeNow+"\","
   			+ "\"contact\":{"
   			+ "\"phone\":\"5556667777\","
   			+ "\"firstName\":\"Johny\","
   			+ "\"lastName\":\"John\"},"
   			+ "\"rating\":5,"
   			+ "\"feedbackText\":\"This is Excellent\","
   			+ "\"consumerConsentToPublish\":true}}";
    ...
    ClientResponse myResponse = postWebResource.type("application/json")
							    .header("X-Api-Key", "GDnxPfrddh5L3DYSUbRv18C6juwHYgLdbxlryQPi")
  								.header("X-Signpost-Webhook-Event-Id", "Email"+randomValue)
    ...
    */
	...
	LOGS.info("Output: "+responseBody);
   	...
   	if((statusCode ==200) || statusCode==201)
   	{
   		...
   		//String responseBody = myResponse.getEntity(String.class);
   		...
   		//ImageName = "SignPost";
		//ImgName = ScreenshotPrinter.screenShot(ImageName, driver, Resultsfolderpath);
		ExpRes = "User Should be able to Trigger Signpost Feedback";
		ActRes = "User Successfully Triggered Signpost Feedback";
		Status = "Pass";
		ExportResults.exportTestResult(
				"SignPost Feedback:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName,
				Resultsfolderpath);
   		 
		VerifyEventDetailsInDBforFeedback(BusinessId, Resultsfolderpath);
  	}
   	else
   	{
   		ExpRes = "User Should be able to Trigger Signpost Feedback";
		ActRes = "There is Some Issue while Triggerring Signpost Feedback";
		Status = "Fail";
		ExportResults.exportTestResult(
				"SignPost Feedback:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName,
				Resultsfolderpath);
	...
	public void VerifyEventDetailsInDBforFeedback(String BusinessId, String Resultsfolderpath) throws Exception
	{
		LOGS.info("Business Id: " + BusinessId);
		...
		for (Document document : documents) {
			if (document.containsValue(BusinessId)) {
				...
				if (document.get("timestamp").toString().contains(presentdate)) {
					...
					if (document.get("eventType").toString().equalsIgnoreCase("Feedback")) {
						...
						ExpRes = "MongoDB: hibuAssistantEventDetails - EventType Should Be Set as Feedback";
						ActRes = "MongoDB: hibuAssistantEventDetails - EventType is set as Feedback: "
