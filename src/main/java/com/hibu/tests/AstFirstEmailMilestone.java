...
public class AstFirstEmailMilestone extends PreRequisites {

	public static Logger LOGS = LogManager.getLogger(AstFirstEmailMilestone.class.getName());
	...
	public void Assistant_FirstEmailMilestone(String BusinessId, String RepName, String Resultsfolderpath) throws Exception
	{
		...
		//--------------- Trigger First Email Milestone -----------------------------
		
		if(astAssetId != null)
		{
			TriggerRestCall_FirstEmailMilestone(astAssetId, BusinessId, Resultsfolderpath);
		}
		else
		{
	...
	//---------------------First Email Milestone --------------------------------------------------------
	
	public void TriggerRestCall_FirstEmailMilestone(String assetID, String BusinessId, String Resultsfolderpath)
			throws Exception {
		...
		String jsonDataFirstEmailMilestone = "{\"eventType\":\"FirstEmailMilestone\"," + "\"eventDetails\":{" + "\"partnerLocationId\":\""
				+ assetID + "\"," + "\"timestamp\":\"" + timeNow + "\"}}";
		...
		request.body(jsonDataFirstEmailMilestone);
		...
		if ((statusCode == 200) || statusCode == 201) {
			...
			ExpRes = "User Should be able to Trigger 1st Email Milestone";
			ActRes = "User Successfully Triggered 1st Email Milestone";
			Status = "Pass";
			ExportResults.exportTestResult(
					"1st Email Milestone:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName, Resultsfolderpath);

			VerifyEventDetailsInDBforFirstEmailMilestone(BusinessId, Resultsfolderpath);
			VerifyNotificationDetailsInDBforFirstEmailMilestone(BusinessId, Resultsfolderpath);
		}
		else
	   	{
	   		ExpRes = "User Should be able to Trigger 1st Email Milestone";
			ActRes = "There is Some Issue while Triggerring 1st Email Milestone";
			Status = "Fail";
			ExportResults.exportTestResult(
					"1st Email Milestone:," + ExpRes + "," + ActRes + "," + Status + "," + ImgName,
					Resultsfolderpath);
	   	}
	...
	public void VerifyEventDetailsInDBforFirstEmailMilestone(String BusinessId, String Resultsfolderpath) throws Exception
	{
		LOGS.info("Business Id: " + BusinessId);
		LOGS.info("Inside VerifyEventDetailsInDBforFirstEmailMilestone");
		...
		for (Document document : documents) {
			if (document.containsValue(BusinessId)) {
				...
				if (document.get("timestamp").toString().contains(presentdate)) {
					...
					if (document.get("eventType").toString().equalsIgnoreCase("FirstEmailMilestone")) {
						...
						ExpRes = "MongoDB: hibuAssistantEventDetails - EventType Should be Set as FirstEmailMilestone";
						ActRes = "MongoDB: hibuAssistantEventDetails - EventType is Set as FirstEmailMilestone: "
	...
	public void VerifyNotificationDetailsInDBforFirstEmailMilestone(String BusinessId, String Resultsfolderpath) throws Exception
	{
		LOGS.info("Business Id: " + BusinessId);
		LOGS.info("Inside VerifyNotificationDetailsInDBforFirstEmailMilestone");
			...
			if (document.containsValue(BusinessId)) {
				...
				try {
					...
					if (document.get("dateTime").toString().contains(presentdate)) {
						...
						if (document.get("eventType").toString().equalsIgnoreCase("FirstEmailMilestone")
								&& document.get("email").toString().contains("We just sent your first email to your contact list")) {

							System.out.println(document);
							System.out.println("Event Type :" + document.get("eventType").toString());
							System.out.println("No. of Documents present :" + documentCounter);

							ExpRes = "MongoDB: hibuAssistantNotificationDetails - EventType Should be Set to FirstEmailMilestone";
							ActRes = "MongoDB: hibuAssistantNotificationDetails - EventType is set as FirstEmailMilestone: "
									+ document.get("eventType");
